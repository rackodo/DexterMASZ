...
public class TransferXp : Command<TransferXp>
{
	...
	public async Task TransferXpCommand(
		[Summary("source", "The user to take the experience from.")] IUser source,
		[Summary("target", "The user to give the experience to.")] IUser target,
		[Summary("textXpCap", "The maximum amount of text experience to transfer.")] long textXpCap = -1,
		...
		)
	{
		...
		{
			await RespondAsync($"No experience to transfer from {source.Mention} to {target.Mention}.", ephemeral: true);
			return;
		}
		...
		var stringify = (GuildUserLevel l) =>
		{
			return $"{l.UserId} ({l.TextXp}, {l.VoiceXp})";
		};
		Console.WriteLine($"Transferred ({textXp}, {voiceXp}) from {stringify(sourceLevel)} to {stringify(targetLevel)}.");
