	...
	private async Task AnnounceUserNote(UserNote userNote, IUser actor, RestAction action)
	{
		...
		if (!string.IsNullOrEmpty(guildConfig.StaffWebhook))
		{
			_logger.LogInformation(
				$"Sending internal webhook for user note {userNote.GuildId}/{userNote.UserId} ({userNote.Id}) to {guildConfig.StaffWebhook}.");
			...
			{
				_logger.LogError(e,
					$"Error while announcing user note {userNote.GuildId}/{userNote.UserId} ({userNote.Id}) to {guildConfig.StaffWebhook}.");
			}
		}
	}
}