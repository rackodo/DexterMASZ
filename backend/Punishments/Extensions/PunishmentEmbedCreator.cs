...
{
    public static async Task<Tuple<EmbedBuilder, ComponentBuilder>> CreateNewModCaseEmbed(this ModCase modCase,
        GuildConfig config,
        AppSettings settings, AnnouncementResult result, DiscordRest rest, IServiceProvider provider)
    {
        ...
        var buttons = new ComponentBuilder().WithButton("View Case", style: ButtonStyle.Link, url: url);

        return new Tuple<EmbedBuilder, ComponentBuilder>(embed, buttons);
    }

    public static async Task<EmbedBuilder> CreateModCaseEmbed(this ModCase modCase, RestAction action, DiscordRest rest,
    ...
    public static async Task<EmbedBuilder> CreateReportEmbed(this IMessage message, IUser user,
        IServiceProvider provider)
    {
        ...
        IUser actor, IServiceProvider provider)
    {
        ...
        var embed = (await EmbedCreator.CreateActionEmbed(action, provider, actor))
            ...
            .WithFooter(
                $"{translator.Get<BotTranslator>().UserId()}: {actor.Id} | {translator.Get<PunishmentTranslator>().CaseId()}: {modCase.CaseId}")
