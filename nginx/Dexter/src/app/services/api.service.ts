  ...
  private handleError(error: HttpErrorResponse, toastr: ToastrService): Observable<never> {
    ...
    if (error.error instanceof ErrorEvent) {
      ...
      if (typeof error.error === 'object' && error.error !== null) {
        ...
        } else if ('customDexterError' in error.error) {
          if (this.apiErrors.find(x => x.key === error.error.customDexterError)) {
            toastr.error(this.apiErrors?.find(x => x.key === error.error.customDexterError)?.value ?? error.error?.message ?? 'Unknown');
          } else {
